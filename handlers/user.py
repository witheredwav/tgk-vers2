    ...
    try:
        if result["tier_changed"] and result["code"]:
            text = (
                ...
                f"После использования кода будет списано ровно {result['tier_threshold']} друзей "
                f"(порог этого уровня) — если у тебя наберётся больше, остаток продолжит копиться._"
            )
        else:
            text = (
